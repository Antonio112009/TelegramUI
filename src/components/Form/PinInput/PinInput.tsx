...
export const PinInput = forwardRef<HTMLElement, PinInputProps>(
  (
    ...
    const { setInputRefByIndex, value, handleButton, onElementClick, PINS } =
      usePinInput({
        value: valueProp,
        onChange,
        onBiometricAuth,
      ...
      <RootRenderer>
        ...
        >
          <header className={styles.header}>
            <Headline className={styles.title} weight="2">
              {label}
            </Headline>
            ...
            {createChunks(PINS, 3).map((rows) => (
              ...
              >
                {rows.map((element) => {
                  ...
                  const ActionButtonComponent = getActionButtonComponent(
                    element,
                    biometricType,
                  );
